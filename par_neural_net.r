...
# for (j in 1:3)
	for (j in 1:length(myFiles))
	{
		...
		nn_model <- foreach(k = 1:nrow(ann_parameters),.packages="nnet") %dopar% {
						...
						nnet(x = a[,-c(1:3)], y = (a[,2]), size=ann_parameters[k,1], decay=ann_parameters[k,2], maxit=ann_parameters[k,3])
						}
		stopCluster(cl)
		...
		ann_val <- foreach(k = 1:nrow(ann_parameters), .combine='cbind', .packages="nnet") %dopar% {
			predict(nn_model[[k]],val_data[,-c(1:3)])
					}                
		stopCluster(cl)
		...
		ann_test <- foreach(k = 1:nrow(ann_parameters), .combine='cbind', .packages="nnet") %dopar% {
			predict(nn_model[[k]],test_data[,-c(1:3)])
						}                
		stopCluster(cl)
